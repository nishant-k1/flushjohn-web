...
function checkSEO(html, url) {
    ...
    checks.push({
      ...
      note:
        desc.length >= 120 && desc.length <= 160
          ? "Optimal length"
          : desc.length < 120
            ? "Too short (aim for 120-160 chars)"
  ...
  checks.push({
    ...
    note:
      ogCount >= 3
        ? "Good coverage"
        : `Missing ${4 - ogCount} recommended tags`,
  });
  ...
  checks.push({
    ...
    note:
      twitterCount >= 2 ? "Good coverage" : "Consider adding Twitter Card tags",
  });
  ...
  checks.push({
    ...
    note:
      h1Count === 1
        ? "Perfect (one H1 per page)"
        : h1Count > 1
          ? "Multiple H1s - consider using only one"
          : "No H1 tag found",
  });
  ...
  checks.push({
    ...
    note:
      altPercentage >= 80
        ? "Good coverage"
        : "Consider adding alt text to more images",
  });
  ...
  checks.push({
    ...
    note: lang
      ? "Language specified"
      : "Consider adding lang attribute to <html>",
  });
...
async function main() {
  ...
  try {
    ...
    log(
      "\n💡 Tip: View the raw HTML in your browser (View Page Source)",
      "blue"
    );
